...
#
# Copyright (C) 2011-2013, 2015, 2020-2021, 2024-2025 Seecr (Seek You Too B.V.) https://seecr.nl
...
class InternalServerTest(IntegrationTestCase):
    ...
    def testListInvalidRecordsForOneRepository(self):
        ...
        header, result = getRequest(
            self.harvesterInternalServerPortNumber,
            "/invalid",
            {"domainId": "adomain", "repositoryId": "integrationtest"},
            parse=True,
        )
        self.assertEqual(
            [
        ...
        self.assertEqual(
            "/page/invalidRecord/?recordId=oai%3Arecord%3A01&domainId=adomain&repositoryId=integrationtest",
            result.xpath("/html/body/div/table/tr/td[@class='link']/a")[0].attrib[
                "href"
            ],
        )
        self.assertEqual(
            "/page/showHarvesterStatus/show?domainId=adomain&repositoryId=integrationtest",
            result.xpath("/html/body/div/p/a/@href")[0],
        )
    ...
    def testViewInvalidRecordWithoutDetails(self):
        ...
        header, result = getRequest(
            ...
            "/invalidRecord",
            {
                ...
                "recordId": "oai:record:02/&gkn",
            },
            parse="lxml",
        )
        self.assertEqual(
            "Repository integrationtest - Record oai:record:02/&gkn",
            result.xpath("//h3/text()")[0],
        )
        self.assertEqual(
            "/page/invalid/?domainId=adomain&repositoryId=integrationtest",
            result.xpath("/html/body/div/p/a/@href")[0],
        )
        self.assertEqual(
            ["No error message."], result.xpath("/html/body/div/pre/text()")
        )
    ...
    def testViewInvalidRecord(self):
        ...
        header, result = getRequest(
            ...
            "/invalidRecord",
            {
                "domainId": "adomain",
                "repositoryId": "integrationtest",
                "recordId": "oai:record:01",
            },
            parse=True,
        )
        self.assertEqual(
            "Repository integrationtest - Record oai:record:01",
            result.xpath("//h3/text()")[0],
        )
        self.assertEqual(
            "/page/invalid/?domainId=adomain&repositoryId=integrationtest",
    ...
    def testGetStatusForDomainAndRepositoryId(self):
        ...
        self.assertEqual(
            "IntegrationTest", data["response"]["GetStatus"][0]["repositoryGroupId"]
        )
    ...
    def testGetStatusForDomain(self):
        ...
        header, result = getRequest(
            self.harvesterInternalServerPortNumber,
            "/get",
            {"verb": "GetStatus", "domainId": "adomain"},
            parse=False,
        )
        data = JsonDict.loads(result)
    ...
    def testGetStatusForDomainAndRepositoryGroup(self):
        ...
        header, result = getRequest(
            ...
            "/get",
            {
                "verb": "GetStatus",
                "domainId": "adomain",
                "repositoryGroupId": "IntegrationTest",
            },
            parse=False,
        )
        data = JsonDict.loads(result)
    ...
    def testGetRepositoriesForDomain(self):
        header, result = getRequest(
            self.harvesterInternalServerPortNumber,
            "/get",
            {"verb": "GetRepositories", "domainId": "adomain"},
            parse=False,
        )
        data = loads(result)
        self.assertEqual(2, len(data["response"]["GetRepositories"]))
        self.assertEqual(
            ["integrationtest", "repository2"],
            [r["identifier"] for r in data["response"]["GetRepositories"]],
        )
    ...
    def testGetRepository(self):
        ...
        data = JsonDict.loads(result)
        self.assertEqual(
            "IntegrationTest", data["response"]["GetRepository"]["repositoryGroupId"]
        )
    ...
    def testRssForHarvesterStatus(self):
        ...
        self.assertEqual(
            "Harvester status voor repository integrationtest",
            xpath(result, "/rss/channel/title/text()")[0],
        )
        self.assertEqual(
            "Recente repository harvest status voor repository integrationtest in adomain",
            xpath(result, "/rss/channel/description/text()")[0],
        )
        self.assertEqual(
            "http://localhost:9999/showHarvesterStatus/adomain/IntegrationTest/integrationtest",
            xpath(result, "/rss/channel/link/text()")[0],
        )
        self.assertEqual(str(60 * 6), xpath(result, "/rss/channel/ttl/text()")[0])

        self.assertEqual(
            "Harvester status voor integrationtest",
    ...
    def testRssForNeverHarvestedRepository(self):
        header, result = getRequest(
            self.harvesterInternalServerPortNumber,
            "/rss",
            {"domainId": "adomain", "repositoryId": "repository2"},
            parse="lxml",
        )
        self.assertEqual(
            "Harvester status voor repository repository2",
            xpath(result, "/rss/channel/title/text()")[0],
        )
    ...
    def testRssForStatusChangesOk(self):
        ...
        self.assertEqual(
            "Harvest status changes for domain 'adomain'",
            xpath(result, "/rss/channel/title/text()")[0],
        )
        self.assertEqual(
            "Status changes per repository for domain 'adomain'",
            xpath(result, "/rss/channel/description/text()")[0],
        )
        self.assertEqual(
            "http://localhost:9999/showHarvesterStatus/adomain",
            xpath(result, "/rss/channel/link/text()")[0],
        )
        self.assertEqual(str(60 * 6), xpath(result, "/rss/channel/ttl/text()")[0])
    ...
    def testRssForStatusChangesError(self):
        self.controlHelper(
            action="raiseExceptionOnIds", id=["%s:oai:record:01" % REPOSITORY]
        )
        self.startHarvester(repository=REPOSITORY)
        header, result = getRequest(
            self.harvesterInternalServerPortNumber,
        ...
        self.assertEqual(1, len(items))
        self.assertEqual(
            "integrationtest: Error", "".join(xpath(items[0], "title/text()"))
        )
        description = "".join(xpath(items[0], "description/text()"))
        self.assertTrue(description.startswith("Harvest time: %s" % TODAY), description)
