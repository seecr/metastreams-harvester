...
# Copyright (C) 2012-2013, 2015, 2020-2022, 2024 Seecr (Seek You Too B.V.) https://seecr.nl
...
class HarvesterTest(IntegrationTestCase):
    def setUp(self):
        ...
        try:
            self.harvesterData.addRepositoryGroup(
                identifier=REPOSITORYGROUP, domainId=DOMAIN
            )
        except ValueError:
            pass
    ...
    def saveRepository(
        ...
        baseUrl = (
            baseUrl
            if baseUrl
            else "http://localhost:%s/oai" % self.helperServerPortNumber
        )
        try:
    ...
    def testHarvestReturnsErrorWillNotSaveState(self):
        ...
        try:
            ...
            self.assertEqual(
                {"verb": ["ListRecords"], "metadataPrefix": ["not_existing"]},
                logs[0]["arguments"],
            )
    ...
    def testHarvestToSruUpdate(self):
        ...
        self.assertEqual(
            ...
            len(
                [
                    f
                    for f in listdir(self.dumpDir)
                    if "info:srw/action/1/delete" in open(join(self.dumpDir, f)).read()
                ]
            ),
        )
        ...
        self.assertEqual("/oai", logs[-1]["path"])
        self.assertEqual(
            {"verb": ["ListRecords"], "resumptionToken": [token]}, logs[-1]["arguments"]
        )
    ...
    def testContinuousHarvest(self):
        ...
        t = Thread(
            target=lambda: self.startHarvester(
    ...
    def testClear(self):
        ...
        header, data = getRequest(
            self.harvesterInternalServerPortNumber,
            "/get",
            {"verb": "GetStatus", "domainId": DOMAIN, "repositoryId": REPOSITORY},
        )
    ...
    def testRefresh(self):
        ...
        log.startRepository()
        for uploadId in [
            "%s:oai:record:%02d" % (REPOSITORY, i) for i in [1, 7, 120, 121]
        ]:
            log.notifyHarvestedRecord(uploadId)
            log.uploadIdentifier(uploadId)
        for uploadId in [
            "%s:oai:record:%02d" % (REPOSITORY, i) for i in [4, 5, 122, 123]
        ]:
            log.notifyHarvestedRecord(uploadId)
        ...
        deleteFiles = [
            join(self.dumpDir, f) for f in listdir(self.dumpDir) if "_delete" in f
        ]
        deletedIds = set(
            [
    ...
    def testInvalidIgnoredUptoMaxIgnore(self):
        ...
        invalidDataMessagesDir = join(
            self.harvesterLogDir, DOMAIN, "invalid", REPOSITORY
        )
        self.assertEqual(maxIgnore + 1, len(listdir(invalidDataMessagesDir)))
        invalidDataMessage01 = open(
            join(invalidDataMessagesDir, "oai:record:01")
        ).read()
        self.assertTrue(
            'uploadId: "integrationtest:oai:record:01"', invalidDataMessage01
        )
    ...
    def testHarvestToFilesystemTarget(self):
        ...
        self.assertEqual(
            8, len(listdir(join(self.filesystemDir, REPOSITORYGROUP, REPOSITORY)))
        )
        self.assertEqual(
    ...
    def testClearOnFilesystemTarget(self):
        ...
        self.saveRepository(
            DOMAIN, REPOSITORY, REPOSITORYGROUP, targetId="FILESYSTEM", action="clear"
        )
        self.startHarvester(repository=REPOSITORY)
        self.assertEqual(
            0, len(listdir(join(self.filesystemDir, REPOSITORYGROUP, REPOSITORY)))
        )
        self.assertEqual(
            set(
                [
    ...
    def testHarvestWithError(self):
        ...
        self.controlHelper(
            action="raiseExceptionOnIds", id=["%s:oai:record:12" % REPOSITORY]
        )
        self.startHarvester(repository=REPOSITORY)
    ...
    def testClearWithError(self):
        ...
        self.saveRepository(DOMAIN, REPOSITORY, REPOSITORYGROUP, action="clear")
        self.controlHelper(
            action="raiseExceptionOnIds", id=["%s:oai:record:05" % REPOSITORY]
        )
        self.emptyDumpDir()
    ...
    def testRefreshWithIgnoredRecords(self):
        ...
        log.startRepository()
        for uploadId in [
            "%s:oai:record:%02d" % (REPOSITORY, i) for i in [1, 2, 120, 121]
        ]:
            ...
            log.uploadIdentifier(uploadId)
        for uploadId in [
            "%s:oai:record:%02d" % (REPOSITORY, i) for i in [4, 5, 122, 123, 124]
        ]:
            log.notifyHarvestedRecord(uploadId)
            log.deleteIdentifier(uploadId)
        for uploadId in [
            "%s:oai:record:%02d" % (REPOSITORY, i) for i in [7, 8, 125, 126, 127, 128]
        ]:
            log.notifyHarvestedRecord(uploadId)
    ...
    def testConcurrentHarvestToSruUpdate(self):
        ...
        for f in requestsLogged:
            ...
            repositoryIds.append(
                xpath(lxml, "//ucp:recordIdentifier/text()")[0].split(":", 1)[0]
            )
    ...
    def testStartHarvestingAddedRepository(self):
        ...
        stdoutfile = join(
            self.integrationTempdir, "stdouterr-meresco-harvester-harvester.log"
        )
        sleepWheel(5)
    ...
    def testErrorReportedToGustos(self):
        ...
        self.saveRepository(
            DOMAIN, REPOSITORY, REPOSITORYGROUP, baseUrl="file://{}".format(baseUrl)
        )
        ...
        sleepWheel(5)
        last_logs = [
            JsonDict.loads(l)["data"] for l in self.gustosUdpListener.log()[errorCount:]
        ]
        for data in reversed(last_logs):
            my_group_log = data.get(f"Harvester ({DOMAIN})", {}).get(
                f"{REPOSITORYGROUP}:{REPOSITORY}"
            )
    ...
    def testCompleteInOnAttempt(self):
        self.saveRepository(DOMAIN, REPOSITORY, REPOSITORYGROUP, complete=True)
        stdouterrlog = self.startHarvester(
            repository=REPOSITORY, runOnce=True, timeoutInSeconds=5
        )
        self.assertEqual(15, self.sizeDumpDir())
        self.assertTrue(
            "Repository will be completed in one attempt" in stdouterrlog, stdouterrlog
        )

    def testHarvestingContinues4Ever(self):
        try:
            self.startHarvester(
                repository=REPOSITORY, runOnce=False, timeoutInSeconds=5
            )
    ...
    def testBadOai(self):
        header, data = getRequest(
            ...
            arguments=dict(verb="ListRecords", metadataPrefix="prefix"),
        )
        self.assertEqual(
            "resume0",
            xpathFirst(data, "/oai:OAI-PMH/oai:ListRecords/oai:resumptionToken/text()"),
        )
        header, data = getRequest(
            port=self.helperServerPortNumber,
            path="/badoai/responsedate",
            arguments=dict(verb="ListRecords", resumptionToken="resume0"),
        )
        self.assertEqual(
            "resume1",
            xpathFirst(data, "/oai:OAI-PMH/oai:ListRecords/oai:resumptionToken/text()"),
    ...
    def testNormalHarvesting(self):
        self.assertEqual(
            "Harvested.", what_happened(self.startHarvester(repository=REPOSITORY))
        )
        ...
        self.assertEqual(15, self.sizeDumpDir())
        self.assertEqual(
            "Nothing to do!", what_happened(self.startHarvester(repository=REPOSITORY))
        )
        self.assertEqual(15, self.sizeDumpDir())
...
def what_happened(output):
    return (
        [
            ...
            if line.strip() and not "Sleeping" in line
        ][-1]
        .split("]")[-1]
        .strip()
    )
