...
class HarvesterLogTest(SeecrTestCase):
    ...
    def testSameDate(self):
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="someuni"
        ) as logger:
    ...
    def testHasWorkBeforeAndAfterDoingWork(self):
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="name"
        ) as logger:
            self.assertTrue(logger.hasWork())
    ...
    def testHasWorkBeforeAndAfterDoingWorkContinuous(self):
        ...
        with harvesterLog(
            ...
            logger.startRepository()
            logger.endRepository(
                None, "2000-01-02T03:04:05Z"
            )  # ignore what repository says as responseDate
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="name"
        ) as logger:
    ...
    def testHasWorkWithResumptionTokenContinuous(self):
        ...
        with harvesterLog(
            ...
            logger.startRepository()
            logger.endRepository(
                "resumptionToken2", logger._state.getZTime().add(seconds=-61).zulu()
            )
    ...
    def testLoggingAlwaysStartsNewline(self):
        ...
        with open(self.stateDir + "/name.stats", "w") as f:
            f.write(
                'Started: 2005-01-02 16:12:56, Harvested/Uploaded/Total: 199/200/1650, Don"crack"'
            )
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="name"
        ) as logger:
    ...
    def testLogLine(self):
        ...
        self.assertEqual(
            "Harvested/Uploaded/Deleted/Total: 3/1/1/0, ResumptionToken: None", comments
        )
    ...
    def testLogLineError(self):
        ...
        self.assertEqual("name", id)
        self.assertTrue(
            comments.startswith('Traceback (most recent call last):|File "')
        )
        self.assertTrue('harvesterlogtest.py", line ' in comments)
        self.assertTrue(
            comments.endswith(
                ', in testLogLineError raise Exception("FATAL")|Exception: FATAL'
            )
        )

    def testLogWithoutDoubleIDs(self):
        ids = Ids(self.stateDir + "/name.ids")
        for anId in ["id:1", "id:2", "id:1"]:
            ids.add(anId)
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="name"
        ) as logger:
    ...
    def testLogIgnoredIdentifierWarning(self):
        with harvesterLog(
            ...
            with open(self.logDir + "/name.events") as fp:
                self.assertTrue(
                    fp.read().endswith("\tWARNING\t[repoid:oai:bla/bla]\tIGNORED\n")
                )
    ...
    def testLogInvalidData(self):
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="repo/id"
        ) as logger:
            logger.startRepository()
    ...
    def testMarkDeleted(self):
        with open(self.stateDir + "/name.stats", "w") as f:
            f.write(
                "Started: 2005-01-02 16:12:56, Harvested/Uploaded/Total: 199/200/1650, Done: 2005-04-22 11:48:30, ResumptionToken: resumption"
            )
        with harvesterLog(
            stateDir=self.stateDir, logDir=self.logDir, name="name"
        ) as logger:
