# aaa jjjjjjjjjjjjjjjjjjjjj# begin license ##
#
# "Metastreams Harvester" is a fork of Meresco Harvester that demonstrates
# the translation of traditional metadata into modern events streams.
#
# Copyright (C) 2011-2012, 2015, 2020-2021, 2024 Seecr (Seek You Too B.V.) https://seecr.nl
...
class RepositoryStatusTest(SeecrTestCase):
    def setUp(self):
        ...
        repoId2LogDir = mkdir(
            self.logDir, self.domainId, "invalid", escapeFilename("repoId/2")
        )
        ...
        def getRepositoryGroupId(domainId, repositoryId):
            return (
                "repoGroupId1"
                if repositoryId in ["repoId1", "repoId/2"]
                else "repoGroupId2"
            )
    ...
    def testGetRunningStatesForDomain(self):
        with open(join(self.stateDir, self.domainId, "repoId1.running"), "w") as fp:
            jsonDump(
                {"changedate": "2012-08-14 12:00:00", "status": "Ok", "message": ""}, fp
            )
        ...
        with open(
            join(self.stateDir, self.domainId, "anotherRepoId.running"), "w"
        ) as fp:
            jsonDump(
                {"changedate": "2012-08-16 12:00:00", "status": "Ok", "message": ""}, fp
            )
            ...
            [
                {
                    ...
                    "recenterrors": [],
                    "invalid": 2,
                    "recentinvalids": ["invalidId1", "invalidId&2"],
    ...
    def testGetStatusForDomainIdAndRepositoryGroupId(self):
        self.assertEqual(
            ...
            self.status.getStatus(
                domainId=self.domainId, repositoryGroupId="repoGroupId1"
            ),
        )
    ...
    def testGetAllInvalidRecords(self):
        ...
        self.assertEqual(["invalidId1", "invalidId&2"], invalidRecords("repoId1"))
    ...
    def testGetInvalidRecord(self):
        def getInvalidRecord(repoId, recordId):
            return tostring(
                self.status.getInvalidRecord(self.domainId, repoId, recordId),
                encoding=str,
            )

        self.assertEqual(
            "<diagnostic>ERROR1</diagnostic>", getInvalidRecord("repoId1", "invalidId1")
        )
        self.assertEqual(
            "<diagnostic>ERROR2</diagnostic>",
            getInvalidRecord("repoId1", "invalidId&2"),
        )
        self.assertEqual(
            "<diagnostic>ERROR3</diagnostic>",
    ...
    def testSucces(self):
        ...
        state = self.status._parseEventsFile(
            domainId=self.domainId, repositoryId="repoId1"
        )
    ...
    def testLotOfErrors(self):
        ...
        self.assertEqual(
            [
                ("2006-03-11T12:19:14Z", "Error 19, Crash"),
