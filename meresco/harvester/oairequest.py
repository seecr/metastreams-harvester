...
# Copyright (C) 2011-2017, 2020-2021, 2025 Seecr (Seek You Too B.V.) https://seecr.nl
# Copyright (C) 2011-2012, 2015, 2020-2021, 2025 Stichting Kennisnet https://www.kennisnet.nl
...
class OaiRequest(object):
    def __init__(self, url, headers=None, _urlopen=None):
        ...
        self._argslist = parse_qsl(
            self._urlElements[QUERY_POSITION_WITHIN_URLPARSE_RESULT]
        )
    ...
    def _request(self, argslist):
        def doUrlopen(context=None):
            return self._urlopen(
                Request(self._buildRequestUrl(argslist), None, self._headers()),
                timeout=5 * 60,
                context=context,
            )
    ...
    def _buildRequestUrl(self, argslist):
        ...
        urlElements[QUERY_POSITION_WITHIN_URLPARSE_RESULT] = urlencode(
...
class OaiResponse(object):
    def __init__(self, response):
        ...
        self.resumptionToken = (
            xpathFirst(
                response, "/oai:OAI-PMH/oai:ListRecords/oai:resumptionToken/text()"
            )
            or ""
        )
        self.responseDate = xpathFirst(response, "/oai:OAI-PMH/oai:responseDate/text()")
        if (
            not self.responseDate is None
        ):  # should be there, happens to be absent for some repositories
...
class OaiRequestException(Exception):
    def __init__(self, url, message):
        Exception.__init__(
            self, 'occurred with repository at "%s", message: "%s"' % (url, message)
        )
        self.url = url
