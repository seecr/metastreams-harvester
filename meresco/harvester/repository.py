...
# Copyright (C) 2015, 2017, 2020-2022, 2026 Seecr (Seek You Too B.V.) https://seecr.nl
...
class Repository(SaharaObject):
    ...
    def shopClosed(self, dateTuple=localtime()[:5]):
        return reduce(
            ...
            [x.areWeWithinTimeslot(dateTuple) for x in self.closedSlots()],
            False,
        )

    def target(self):
        return self._proxy.getTargetObject(
            domainId=self.domainId, identifier=self.targetId
        )

    def mapping(self):
        return self._proxy.getMappingObject(
            domainId=self.domainId, identifier=self.mappingId
        )
    ...
    def do(self, stateDir, logDir, generalHarvestLog=nillogger, gustosClient=None):
        gustosReport = _prepareGustosReport(
            gustosClient, self.domainId, self.repositoryGroupId, self.id
        )
        if not (stateDir or logDir):
            raise RepositoryException("Missing stateDir and/or logDir")
        repoState = self._initState(stateDir, logDir)
        try:
            ...
            if completeHarvest:
                generalHarvestLog.logInfo(
                    "Repository will be completed in one attempt", id=self.id
                )
            ...
            gustosReport(repoState.eventCounts())
            repoState.close()

    def _initState(self, stateDir, logDir):
        return State(stateDir, logDir, self.id)
...
def _prepareGustosReport(client, domain_id, repo_group_id, repo_id):
    ...
    def gustosReport(counts):
        client.report(
            values={
                f"Harvester ({domain_id})": {
                    f"{repo_group_id}:{repo_id}": {
                        k: {COUNT: v} for k, v in counts.items()
                    }
                }
            }
        )
