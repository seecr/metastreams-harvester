...
# Copyright (C) 2015, 2017, 2020-2023, 2025 Seecr (Seek You Too B.V.) https://seecr.nl
...
class Action(object):
    ...
    def create(repository, state, generalHarvestLog):
        ...
        try:
            ...
            return actionClass(
                repository, state=state, generalHarvestLog=generalHarvestLog
            )
        except KeyError:
...
class SmoothAction(Action):
    def do(self):
        ...
        if (
            len(self._state.oldIds) == 0
            and len(self._state.ids) + len(self._state.invalidIds) != 0
        ):
