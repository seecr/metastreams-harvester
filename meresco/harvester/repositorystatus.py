...
#
# Copyright (C) 2011-2012, 2015, 2020-2022, 2025 Seecr (Seek You Too B.V.) https://seecr.nl
...
class RepositoryStatus(Observable):
    ...
    def _getStatus(self, domainId, repositoryGroupId=None, repositoryId=None):
        if repositoryId:
            groupId = self.call.getRepositoryGroupId(
            ...
            groupIds = (
                ...
                else self.call.getRepositoryGroupIds(domainId=domainId)
            )
            for groupId in groupIds:
                repositoryIds = self.call.getRepositoryIds(
                    domainId=domainId, repositoryGroupId=groupId
                )
    ...
    def getRunningStatesForDomain(self, domainId):
        ...
        return sorted(
            [
                mergeDicts(
                    _jsonLoad(filepath),
                    {"repositoryId": repoId, "repositoryGroupId": groupId},
                )
                for groupId in self.call.getRepositoryGroupIds(domainId=domainId)
                for repoId in self.call.getRepositoryIds(
                    domainId=domainId, repositoryGroupId=groupId
                )
                for filepath in [
                    join(
                        self._statePath, domainId, escapeFilename("%s.running" % repoId)
                    )
                ]
                if isfile(filepath)
            ],
    ...
    def getInvalidRecord(self, domainId, repositoryId, recordId):
        ...
        with open(
            join(invalidDir, escapeFilename(repositoryId), escapeFilename(recordId))
        ) as fp:
            return parse(fp)
    ...
    def _getRepositoryStatus(self, domainId, groupId, repoId):
        ...
        return JsonDict(
            ...
            recenterrors=[
                dict(date=error[0], error=error[1]) for error in stats["recenterrors"]
            ],
            invalid=len(self.invalidRecords(domainId, repoId)),
...
def _reformatDate(aDate):
    return (
        aDate[0 : len("YYYY-MM-DD")]
        + "T"
        + aDate[len("YYYY-MM-DD ") : len("YYYY-MM-DD HH:MM:SS")]
        + "Z"
    )
