...
class HarvesterData(object):
    ...
    def get_domain_aliases(self):
        return (
            json.loads(self._domain_alias_file.read_text())
            if self._domain_alias_file.is_file()
            else {}
        )
    ...
    def getRepositoryGroups(self, domainId):
        return [
            self.getRepositoryGroup(repositoryGroupId, domainId)
            for repositoryGroupId in self.getRepositoryGroupIds(domainId)
        ]
    ...
    def addRepositoryGroup(self, identifier, domainId):
        ...
        self._store.addData(
            id_combine(domainId, identifier),
            "repositoryGroup",
    ...
    def getRepositoryIds(self, domainId, repositoryGroupId=None):
        result = JsonList()
        allIds = (
            self.getRepositoryGroupIds(domainId)
            if repositoryGroupId is None
            else [repositoryGroupId]
        )
    ...
    def addRepository(self, identifier, domainId, repositoryGroupId):
        ...
        self._store.addData(
            id_combine(domainId, identifier),
            "repository",
            JsonDict(dict(identifier=identifier, repositoryGroupId=repositoryGroupId)),
        )
        group.setdefault("repositoryIds", []).append(identifier)
        self._store.addData(
            id_combine(domainId, repositoryGroupId), "repositoryGroup", group
        )
    ...
    def add_header(self, domainId, repositoryId, name, value):
        ...
        if name not in repository["headers"]:
            ...
            self._store.addData(
                id_combine(domainId, repositoryId), "repository", repository
            )
    ...
    def addClosingHours(self, identifier, domainId, week, day, startHour, endHour):
        ...
        shopClosed.append(
            str(Timeslot(f"{week}:{day}:{startHour}:00-{week}:{day}:{endHour}:00"))
        )
    ...
    def repositoryDone(self, identifier, domainId):
        ...
        self._store.addData(
            id_combine(domainId, identifier), "repository", repository, newId=False
        )
